...
def select_image(batch_number, batch_sample, path):
    ...
    while 1:
        ...
        for offset  in range(0, sum_lines, batch_size):
            ...
            for batch_sample in batch_samples:
                img_name, angle = select_image(batch_number, batch_sample, path)
                print(img_name)
                image = cv2.imread(img_name)
                images, angles = adjust_steering(image, angle, images, angles)
                batch_number += 1
            X_train = np.array(images)
            y_train = np.array(angles)
